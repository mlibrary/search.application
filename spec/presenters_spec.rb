RSpec.describe Search::Presenters do
  ...
  context "#add_param" do
    it "adds the filter param to a give url" do
      ...
      expected = "#{uri}&filter.format=Audio%20(music)"

      expect(subject.add_param(uri: uri, uid: "format", value: "Audio (music)", prefix: "filter")).to eq(expected)
    end
  ...
  context "#remove_param" do
    it "removes the filter param when it exists to a give url" do
      ...
      expect(subject.remove_param(uri: uri, uid: "format", value: "Audio (music)", prefix: "filter")).to eq(expected)
    end
    it "returns the same url when the filter param isn't there" do
      expected = URI.parse("#{S.base_url}/catalog?query=whatever blah")

      expect(subject.remove_param(uri: expected, uid: "format", value: "Audio (music)", prefix: "filter")).to eq(expected.to_s)
    end
  end
end
